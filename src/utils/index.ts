  ...
  if (questionType == 1) {
    ...
    if (Array.isArray(choices)) {
      ...
      const foundChoice = choices.find(
        (choice) => (choice as AnswerChoiceType1_2).chooseIndex === indexAnswer
      );
      const check = foundChoice
        ? (foundChoice as AnswerChoiceType1_2).isCorrect
        : null;
...
export const checkCorrectAnswer = ({
  ...
  if (partIndex in (answerChoices || {})) {
    if (
      ...
      return (answerChoices[partIndex][questionIndex] as AnswerChoiceType1_2)
        ?.isCorrect;
    } else if (
      questionIndex in answerChoices[partIndex] &&
      ...
      if (
        (
          answerChoices?.[partIndex]?.[questionIndex] as AnswerChoiceType3[]
        )?.every?.(
          (itemQuestionAnswer) =>
