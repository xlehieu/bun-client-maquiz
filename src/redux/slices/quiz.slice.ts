...
export const fetchMyListQuiz = createAsyncThunk(
  "quiz/fetchMyQuiz",
  async (params?: Partial<IQuerySkipLimit>) => {
    try {
...
export const quizSlice = createSlice({
  ...
  reducers: {
    ...
    setQuizOfQuizDetail: (
      state,
      action: PayloadAction<Omit<BodyUpsertQuestionQuiz, "quizId">>
    ) => {
      if (state.quizDetail?.quiz) {
        ...
        const partChange = state.quizDetail.quiz.find(
          (part) => part.partId === action.payload.partId
        );
        if (!partChange) {
          ...
          if (!questionChange) {
            partChange.questions.push(
              question as MatchQuestion | QuestionType_1_2
            );
          } else {
            if ([1, 2].includes(questionType)) {
              (questionChange as QuestionType_1_2).questionContent = action
                .payload.questionContent as string;
              (questionChange as QuestionType_1_2).answers = action.payload
                .answers as AnswerType_1_2[];
            } else if (questionType === 3) {
    ...
    changeQuizPartName: (
      ...
      const part = state.quizDetail?.quiz.find(
        (part) => part.partId === action.payload.partId
      );
      console.log(part);
      if (part) {
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchQuizDetail.fulfilled, (state, action) => {
        ...
        const questionId =
          action.payload?.quiz?.[0]?.questions?.[0]?.questionId;
        if (partId) {
