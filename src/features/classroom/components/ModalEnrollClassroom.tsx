...
const ModalEnrollClassroom = ({ isOpen, onClose }: Props) => {
  ...
  const enrollInClassMutation = useMutationHooks(
    (data: { classCode: string }) => enrollInClassroom(data)
  );
  ...
  useEffect(() => {
    if (enrollInClassMutation.isSuccess) {
      ...
      router.push(
        `${USER_DASHBOARD_ROUTER.CLASSROOM}/${formEnrollClassroom.getFieldValue(
          "classCode"
        )}`
      );
