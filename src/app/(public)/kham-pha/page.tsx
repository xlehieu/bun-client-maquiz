...
const QuizzesProvider = ({ children }: any) => {
  ...
  const buildQuery = () => {
    ...
    if (educationLevel && educationLevel.length > 0) {
      params.push(
        `educationLevel=${educationLevel.map(encodeURIComponent).join(",")}`
      );
    }
  ...
  return (
    <aside className="z-10 bg-white w-56 shadow-lg hidden md:block rounded py-5">
      ...
      <div className="flex flex-col mx-4 mt-3 text-[#333]">
        <p>
          <FontAwesomeIcon className="text-camdat" icon={faCalendarDays} /> Năm
          học
        </p>
        <Slider
          value={schoolYear}
  ...
  return (
    <QuizzesProvider>
      <div className="relative">
        <h1 className="text-xl font-medium text-gray-700 pb-3">
          Khám phá đề thi
        </h1>
