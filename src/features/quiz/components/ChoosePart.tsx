...
const ChoosePart = () => {
  ...
  const { quizDetail, currentQuizPartId } = useAppSelector(
    (state) => state.quiz
  );
  ...
  const handleAddQuizPartName = (formValue: FormField) => {
    ...
    if (formValue.isChange) {
      dispatch(
        changeQuizPartName({ partId: currentQuizPartId, partName: value })
      );
  ...
  const handleClickChoosePart = (quizPart: QuizPart) => {
    if (currentQuizPartId === quizPart.partId) {
      ...
      if (firstQuestionId)
        dispatch(setCurrentUpdateQuestionQuizId(firstQuestionId));
    }
  };
    ...
    <Fragment>
      ...
      >
        <Form
          form={formPart}
          layout="vertical"
          onFinish={handleAddQuizPartName}
          className="mt-4"
        >
          ...
          <Form.Item
            name="partName"
            label={
              <span className="font-bold text-slate-600">Tên phần thi</span>
            }
