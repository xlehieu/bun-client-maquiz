import {
  IGetAndEnrollClassroomDetail,
  IUpdateInfoClassroom,
} from "@/interface";
import axiosCredentials from "@/config/axios.credential";
import { ApiResponse } from "@/@types/api.type";
import {
  BodyCreateClassroom,
  ClassroomDetailRecord,
...
export const getClassroomDetail = async (classCode: string) => {
  const res = await axiosCredentials.get<ApiResponse<ClassroomDetailRecord>>(
    `/classrooms/detail/${classCode}`
  );
