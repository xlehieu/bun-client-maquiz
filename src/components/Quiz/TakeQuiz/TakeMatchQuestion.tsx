...
// }
{
  ...
  useEffect(() => {
    ...
    const answerValue =
      answerMatchingQuestion?.[currentPartIndex]?.[currentQuestionIndex]?.map(
        (item: any) => {
          if (item.question && item.answer) {
            return {
              start: `question-${item.question}`,
              end: `answer-${item.answer}`,
            };
          }
        }
      ) || [];
  ...
  const handleMouseUp = (e: React.MouseEvent<HTMLDivElement>, match: any) => {
    ...
    setXArrows((prevValue: any[]) => {
      ...
      const idxArr = newValue?.findIndex(
        (item) => item?.start === elementCurrentMouseDown
      );
  ...
  const handleConfirm = () => {
    const matchQuestion =
      answerMatchingQuestion?.[currentPartIndex]?.[currentQuestionIndex];
    dispatchAnswerChoices({
      ...
      <div className="relative w-full" ref={containerRef}>
        <div className="grid grid-cols-2 gap-32">
          <div className="flex flex-col gap-10">
            {shuffleMatchQuestion?.[currentPartIndex]?.[
              currentQuestionIndex
            ]?.matchQuestions?.map((matchQuestion: any, index: number) => (
              <div
          ...
          <div className="flex flex-col gap-10">
            {shuffleMatchQuestion?.[currentPartIndex]?.[
              currentQuestionIndex
            ]?.matchAnswers?.map((matchAnswer: any, index: number) => (
              <div
                key={index + Math.random()}
                ref={(el) => {
                  refQuestion.current[index] = el;
