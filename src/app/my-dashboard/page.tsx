...
const MyLibrary = () => {
  ...
  const { isFetchingExamHistory, examHistory } = useAppSelector(
    (state) => state.user
  );
  useEffect(() => {
  ...
  return (
    <>
      <section className="pb-5">
        <h1 className="text-3xl font-bold text-pink-300 text-center my-5">
          Đề thi yêu thích
        </h1>
      ...
      <section className="pb-5">
        <h1 className="text-3xl font-bold text-amber-950 text-center my-5">
          Lịch sử làm bài thi
        </h1>
        <div className="bg-white px-3 pt-3 pb-10 border rounded-lg shadow-md">
            ...
            {examHistory?.map((exam: any, index: number) => (
              <div className="flex border rounded" key={index}>
                ...
                <div className="flex flex-col">
                  ...
                  <p className="text-lg">
                    Điểm:{" "}
                    <span className="text-red-600 text-2xl">{exam.score}</span>
                  </p>
                </div>
              </div>
            ))}
