...
const CreateGeneralInfoQuiz = () => {
  ...
  const createQuizGeneralInfoMutation = useMutationHooks(
  ...
  useEffect(() => {
    if (
      createQuizGeneralInfoMutation.isSuccess &&
      createQuizGeneralInfoMutation.data
    ) {
      // dispatch(setCurrentCreateQuizId(createQuizGeneralInfoMutation?.data?._id));
      toast.success("Tạo bài trắc nghiệm thành công");
      router.replace(
        `${USER_DASHBOARD_ROUTER.EDIT_MY_QUIZ_NO_PARAMS}${createQuizGeneralInfoMutation?.data?._id}`
      );
    ...
    createQuizGeneralInfoMutation.isSuccess,
    createQuizGeneralInfoMutation.isError,
  ]);

  const handleCreateQuiz = (formValue: BodyCreateGeneralInformationQuiz) => {
  ...
  return (
    <Card>
      <Form
        form={form}
        initialValues={{ isUseChatBot: false }}
        layout="vertical"
      ...
      >
            ...
            <Col xs={24} lg={16}>
              <div className="space-y-5">
                <Form.Item
                  name="name"
                  label={
                    <span className="font-bold text-slate-600">Tên đề thi</span>
                  }
                  rules={[
                    { required: true, message: "Tên đề thi là bắt buộc" },
                  ]}
                >
                  <Input
                ...
                <Row gutter={[12, 0]} className="m-0">
                  <Col span={12}>
                    <Form.Item
                      ...
                      rules={[
                        { required: true, message: "Năm học là bắt buộc" },
                      ]}
                    >
                      <Input type="number" className="" placeholder="Năm học" />
                    </Form.Item>
                  </Col>
                  <Col span={12}>
                    <Form.Item
                ...
                <Form.Item<BodyCreateGeneralInformationQuiz>
                  name="school"
                  label={
                    <span className="font-bold text-slate-600">Trường học</span>
                  }
                  rules={[
                    { required: true, message: "Vui lòng chọn trình độ" },
                  ]}
                >
                  <Select className="" placeholder="Chọn trường học">
                    {UNIVERSITIES?.map((schoolName) => (
                      <Select.Option value={schoolName} key={schoolName}>
                        {schoolName}
                      </Select.Option>
                    ))}
                  </Select>
                ...
                <Form.Item
                  ...
                  rules={[
                    { required: true, message: "Chọn ít nhất một môn học" },
                  ]}
                >
                  <Select
                    mode="multiple"
                ...
                <Form.Item
                  ...
                  rules={[
                    { required: true, message: "Mô tả bài thi là bắt buộc" },
                  ]}
                >
                  <TextArea
                    rows={4}
        ...
        <div className="flex justify-end pt-6 border-t border-slate-100">
          <Button
            onClick={form.submit}
            disabled={createQuizGeneralInfoMutation.isPending}
