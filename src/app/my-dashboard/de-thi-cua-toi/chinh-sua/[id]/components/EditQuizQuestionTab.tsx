...
const EditQuizQuestionTab = () => {
  ...
  const { currentQuizPartId, currentUpdateQuestionQuizId, quizDetail } =
    useAppSelector((state) => state.quiz);

  const [form] = Form.useForm<BodyUpsertQuestionQuiz>();
  const questionTypeWatch = Form.useWatch("questionType", form);

  const updateQuestionMutation = useMutationHooks(
    (data: BodyUpsertQuestionQuiz) => updateQuizQuestion(data)
  );
  ...
  useEffect(() => {
    const question = quizDetail?.quiz
      ...
      ?.questions.find(
        (item) => item.questionId === currentUpdateQuestionQuizId
      );
  ...
  const handleSubmitCreateQuestion = async (
    formValues: BodyUpsertQuestionQuiz
  ) => {
    if (
      !quizDetail?._id ||
      !currentQuizPartId ||
      !currentUpdateQuestionQuizId
    ) {
          ...
          <div className="sticky top-6 space-y-6">
            <div className="bg-slate-50 p-6 rounded-[32px] border border-slate-100 shadow-sm">
              <div className="flex items-center gap-3 mb-6">
                ...
                <h5 className="font-bold text-slate-800 m-0">
                  Cấu trúc đề thi
                </h5>
              </div>
              <ChoosePart />
            ...
            onValuesChange={(
              ...
              if (
                ...
                form.setFieldValue("matchQuestions", [
                  { match: 1, questionContent: "", answer: "" },
                ]);
              }
            }}
          >
