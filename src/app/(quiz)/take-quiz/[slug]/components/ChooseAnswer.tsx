    ...
    const timeoutId = setTimeout(() => {
      if (
        ...
        if (currentQuizDetail?.quiz) {
          ...
          if (
            currentQuestionIndex ===
            currentQuizDetail?.quiz?.[currentPartIndex]?.questions?.length - 1
          ) {
            if (currentPartIndex === currentQuizDetail?.quiz?.length - 1)
              return;
            else {
      ...
      // ngược lại nếu chọn đủ các câu thì next
        ...
        if (
          ...
          const lengthAnswerChoicesCurrent = (
            answerChoices[currentPartIndex][currentQuestionIndex] as any
          )?.length;
