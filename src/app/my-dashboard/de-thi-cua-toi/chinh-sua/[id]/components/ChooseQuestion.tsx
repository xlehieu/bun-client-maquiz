...
import { setCurrentUpdateQuestionQuizId, setQuizOfQuizDetail } from '@/redux/slices/quiz.slice';
...
const ChooseQuestion = () => {
    ...
    const questions = quizDetail?.quiz.find((item) => item.partId === currentQuizPartId)?.questions || [];

    const handleAddQuestion = () => {
        const questionId = uuidv7();
        const part = quizDetail?.quiz.find((item) => item.partId === currentQuizPartId);
    ...
    return (
        <div className="animate-in fade-in duration-500">
            ...
            <div className="flex items-center justify-between mb-4">
                <span className="text-xs uppercase tracking-wider font-bold text-slate-400">Câu hỏi</span>
                <button
            ...
            {questions.length > 0 ? (
                <div className="grid grid-cols-5 gap-2">
                    {questions.map((quizContent, index) => {
                        const isActive = quizContent.questionId === currentUpdateQuestionQuizId;
                        return (
                            <button
                                key={quizContent.questionId}
                                onClick={() => dispatch(setCurrentUpdateQuestionQuizId(quizContent.questionId))}
                                className={`
                ...
                <div className="py-8 px-4 border-2 border-dashed border-slate-100 rounded-2xl flex flex-col items-center justify-center text-slate-400">
                    <FontAwesomeIcon icon={faQuestionCircle} className="text-2xl mb-2 opacity-20" />
            ...
            <div className="mt-4 pt-4 border-t border-slate-50 flex justify-between items-center text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                <span>Tổng cộng</span>
                <span className="text-slate-600 bg-slate-100 px-2 py-0.5 rounded-full">{questions.length} câu</span>
            </div>
        </div>
    );
};
