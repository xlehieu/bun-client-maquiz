    ...
    return (
        <Spin spinning={uploadPostMutation.isPending}>
            <div className="glass-card p-6 shadow-sm border border-white/50">
                <Form form={formCreatePost} onFinish={uploadPost} initialValues={{ quizzes: [] }}>
                    ...
                    <div className="mt-6 p-5 rounded-2xl bg-white/30 backdrop-blur-md border border-white/60 shadow-sm">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
                            {quizzWatch?.map((itemWatch) => {
                                ...
                                if (quiz)
                                    return (
                                        <div key={quiz._id} className="transition-transform hover:scale-[1.02]">
                                            <QuizCard quizDetail={quiz} allowEdit={false} showButton={false} />
                                        </div>
                                    );
                                return null;
                            })}
                ...
                <div className="flex justify-between items-center mt-8">
                    <button
                        ...
                        className="soft-button flex items-center gap-2 px-6 py-2.5 text-primary font-semibold hover:bg-blue-50 transition-all active:scale-95"
                    >
