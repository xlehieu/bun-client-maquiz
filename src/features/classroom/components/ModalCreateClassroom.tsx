...
const ModalCreateClassroom = ({ isOpen, onClose }: Props) => {
  ...
  const createClassroomMutation = useMutationHooks(
    (data: { classroomName: string; subjectName: string }) =>
  ...
  useEffect(() => {
    if (createClassroomMutation.isSuccess) {
      ...
      onClose();
      router.push(
        `${USER_DASHBOARD_ROUTER.CLASSROOM}/${createClassroomMutation.data?.classCode}`
      );
