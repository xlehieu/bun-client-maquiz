...
import { Button, Col, DatePicker, Form, InputNumber, Row, Spin, TimePicker, Card } from 'antd';
import dayjs from 'dayjs';
import { useEffect, useState } from 'react';
import { toast } from 'sonner';
import QuizCard from '@/features/quiz/components/QuizCard';
import ModalChooseQuiz from '@/features/quiz/components/ModalChooseQuiz';
...
const UploadExam = () => {
    ...
    const uploadExamMutation = useMutationHooks((data: BodyCreateClassExam) => createClassExam(data));
    const uploadExam = (formValue: BodyCreateClassExam) => {
        ...
        const body: BodyCreateClassExam = {
            ...formValue,
            duration: dayjs(formValue.duration).hour() * 60 + dayjs(formValue.duration).minute(),
        };
        ...
        <div className="max-w-5xl mx-auto">
            <Spin spinning={uploadExamMutation.isPending}>
                ...
                <Card className="mb-6 shadow-lg rounded-2xl border-0 overflow-hidden">
                    <div className="bg-gradient-to-r from-primary to-indigo-600 -m-6 mb-6 p-6 text-white">
                        <h2 className="text-2xl font-bold flex items-center gap-3">
                            <span className="bg-white/20 p-3 rounded-lg backdrop-blur-sm">📝</span>
                            Tạo bài kiểm tra mới
                        </h2>
                        <p className="text-blue-100 mt-2 text-sm">Thiết lập thông tin và thời gian cho bài kiểm tra</p>
                    </div>
                        ...
                        <Row gutter={[20, 20]}>
                            ...
                            <Col xs={24} lg={12}>
                                <Card className="h-full border-2 border-green-100 hover:border-green-300 hover:shadow-md transition-all rounded-xl bg-gradient-to-br from-green-50 to-emerald-50">
                                    <div className="flex items-center gap-2 mb-3">
                                        ...
                                        <span className="font-semibold text-gray-700 text-base">Thời gian bắt đầu</span>
                                    </div>
                            ...
                            <Col xs={24} lg={12}>
                                <Card className="h-full border-2 border-blue-100 rounded-xl bg-gradient-to-br from-blue-50 to-indigo-50">
                                    ...
                                    <Form.Item<BodyCreateClassExam> name="endTime" className="mb-0">
                                        <DatePicker
                                            disabled
                            ...
                            <Col xs={24} lg={12}>
                                <Card className="h-full border-2 border-purple-100 hover:border-purple-300 hover:shadow-md transition-all rounded-xl bg-gradient-to-br from-purple-50 to-pink-50">
                                    <div className="flex items-center gap-2 mb-3">
                                        <span className="text-2xl">🔢</span>
                                        <span className="font-semibold text-gray-700 text-base">Số lần làm bài</span>
                                    </div>
                                    <Form.Item<BodyCreateClassExam>
                                        name="maxAttempts"
                                        rules={[{ required: true, message: 'Vui nhập số lần làm bài' }]}
                                        className="mb-0"
                                    >
                                        <InputNumber
                                            className="w-full h-12 rounded-lg items-center shadow-sm hover:shadow-md transition-shadow flex"
                                            min={0}
                                            max={10}
                                ...
                                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-sm text-gray-500">Chọn đề thi cho bài kiểm tra này</p>
                                        </div>
                                    </div>
                                    <Button
                                        type="primary"
                                        size="large"
                                        className="bg-gradient-to-r from-primary to-indigo-600 hover:from-blue-700 hover:to-indigo-700 border-0 rounded-xl shadow-lg hover:shadow-xl transition-all h-12 px-6 font-semibold"
                                ...
                                {quizWatch ? (
                                    <div className="bg-gradient-to-br from-blue-50 to-indigo-50 p-5 rounded-xl border-2 border-blue-100">
                                        ...
                                        <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-5 gap-3">
                                            {(() => {
                                                const quiz = listMyQuiz.find((item) => item._id === quizWatch);
                                                if (quiz)
                                                    return (
                                                        <QuizCard
                                                            quizDetail={quiz}
                ...
                <div className="flex justify-end gap-3 mt-6">
                    <Button size="large" className="rounded-xl px-8 h-12 font-semibold hover:shadow-md transition-all">
                        Hủy bỏ
                    </Button>
                    <Button
                        type="primary"
