            ...
            setQuiz((preValue: IQuiz) => {
                ...
                if (Number.isInteger(currentPartIndex) && Number.isInteger(currentQuestionIndex) && newQuizDetail?.quiz)
                    newQuizDetail.quiz[currentPartIndex].questions[currentQuestionIndex] = {
                        questionType:
                            newQuizDetail?.quiz?.[currentPartIndex]?.questions?.[currentQuestionIndex]?.questionType,
                        matchQuestions,
                    };
                ...
                {(fields, { remove, add }) => (
                    <Fragment>
                        {fields.map(({ key, name }, index) => (
                            <Fragment>
                                <Row key={key} className="flex items-center justify-between" gutter={[12, 12]}>
                                    <Col xs={11}>
                                        <Form.Item<BodyUpsertQuestionQuiz['matchQuestions']>
                                    ...
                                    <Col xs={11}>
                                        ...
                                        >
                                            <Input type="text" className="w-full" placeholder={`Đáp án ${index + 1}`} />
                                        </Form.Item>
                                    </Col>
