...
const GeneralInformation = ({ quizDetail }: GeneralInformationProps) => {
    ...
    const favoriteMutation = useMutationHooks((data: any) => UserService.favoriteQuiz(data));
    ...
    const buttonGeneralInfo = [
        {
            title: 'Tải file PDF',
            class: 'bg-gradient-to-r from-red-700 to-red-500 text-white rounded hover:opacity-60 duration-300 w-full py-1',
            icon: <FontAwesomeIcon icon={faFilePdf} className="mr-2" />,
            handler: handleExportPdf,
        },
        {
            title: 'Bắt đầu ôn thi',
            class: 'bg-gradient-to-r from-primary to-green-700 text-white rounded hover:opacity-60 duration-300 w-full py-1',
            icon: <FontAwesomeIcon icon={faPlayCircle} className="mr-2" />,
            handler: () => setIsOpenModalSetting(true),
        },
        {
            title: 'Yêu thích',
            class: 'bg-gradient-to-r from-pink-400 to-pink-600 text-white rounded hover:opacity-60 duration-300 w-full py-1',
    ...
    return (
        <>
                ...
                <div className="grid grid-cols-3 mt-2 gap-3">
                    ...
                    <div className="col-span-1">
                        <div className="flex flex-col gap-3">
                            ...
                            {quizDetail?.schoolYear && (
                                <div className="mt-2">
                                    <p className="font-medium">Năm học: {quizDetail?.schoolYear}</p>
                                </div>
                            )}
                            {quizDetail?.educationLevel && quizDetail?.educationLevel?.length > 0 && (
                                <div className="mt-2">
                                    <p className="font-medium">
                                        Trình độ: <span>{quizDetail?.educationLevel.join(', ')}</span>
                                    </p>
                                </div>
                            )}
                    ...
                    {buttonGeneralInfo.map((button, index) => (
                        ...
                        >
                            {button.icon}
                            {button.title}
                            {loading.index === index && loading.isLoading && <LoadingOutlined className="ml-2" />}
                        </button>
                    ))}
                </div>
            </div>
            ...
            >
                ...
                >
                    <Form.Item<FormSetting> name="timePassQuestion" label="Tự động chuyển câu sau">
                        <Select className="w-full md:w-1/2 mt-2" placeholder="Tự động chuyển câu sau">
