...
const ClassPostCard = ({ post, handleOpenModal, viewByAdmin = false }: Props) => {
    ...
    return (
        <div className="bg-white rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 overflow-hidden border border-gray-100">
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div className="flex flex-col">
                        ...
                        <div className="flex items-center gap-2 text-gray-500 text-xs mt-1">
                            <span>🕒</span>
                            <span>{post?.createdAt && dayjs(post?.createdAt).format('DD/MM/YYYY - HH:mm')}</span>
                        </div>
                    </div>
                </div>

                {post?.createdBy?.email === userProfile?.email && !viewByAdmin && (
                    <Dropdown
                        overlay={
            ...
            {/* Content */}
            <div className="p-5 text-gray-700 leading-relaxed">{HTMLReactParser(post?.content || '')}</div>
